...
from .ble_client import IseoClient, UserSubType, IseoAuthError, IseoConnectionError
from .const import (
    CONF_ADDRESS,
    CONF_PRIV_SCALAR,
    CONF_USER_SUBTYPE,
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    async def handle_read_users(call: ServiceCall):
        ...
        # For simplicity, we use the first registered lock.
        # In a multi-lock setup, this could be extended to accept a device_id.
    ...
    hass.services.async_register(
        DOMAIN, "read_users", handle_read_users,
        supports_response=entity_platform.SupportsResponse.ONLY,
    )
    hass.services.async_register(
        DOMAIN, "delete_user", handle_delete_user,
        schema=vol.Schema({
